...
def loss_fn(model, mesh, b_left, b_right, initial, initial_values, threshold = 0.1):
    ...
    boundary_mean = torch.mean(boundary_loss.detach(), axis=1)
    #print(boundary_mean)
    ...
    tsl_rolled[0] = initial_mean
    time_slice_weight = 1.0/(torch.cumsum(tsl_rolled, 0) + 1.0)**4
    ...
    boundary_loss = boundary_loss.reshape(train_points[-1]) * time_slice_weight
    pde_loss = pde_loss * time_slice_weight

    return pde_loss.mean(), boundary_loss.mean(), initial_loss.mean(), time_slice_weight
...
def train_model(
    ...
    all_data["time_slice_weight"] = np.zeros((max_epochs,train_points[1]))
    ...
    for epoch in range(0, max_epochs):
        ...
        def closure():
            optimizer.zero_grad()
            pde_loss, boundary_loss, initial_loss, time_slice_weight = loss_fn(model)
            ...
            if not all_data["closure_calls"][epoch]:
                with torch.no_grad():
                    ...
                    all_data["time_slice_weight"][epoch] = time_slice_weight.cpu().numpy().squeeze()
...
if __name__ == '__main__':
    ...
    for j, model_class in enumerate(MODELS):
        ...
        for init_seed in INIT_SEEDS:
            ...
            # train data
            pd.DataFrame(train_data["time_slice_weight"], columns=b_left.part[1].detach().cpu().numpy()[:,0]).to_csv(os.path.join(seed_folder_name, "time_slice_weight.csv"), index = False)
