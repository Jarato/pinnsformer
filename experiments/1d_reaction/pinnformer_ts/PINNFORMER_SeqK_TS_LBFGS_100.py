...
def create_loss_function(num_points = (51, 51), k = 5, deltaT = 0.001):
    mesh, boundaries = generate_mesh_object(num_points, domain=problem_domain, device=device, full_requires_grad=True, border_requires_grad=False, num_seq_steps=k, seq_step_size=deltaT)
...
def generate_test(num_points = (201, 201), k = 5, deltaT = 0.001):
    test_mesh, _ = generate_mesh_object(num_points, domain=problem_domain, device=device, full_requires_grad=False, border_requires_grad=False, num_seq_steps=k, seq_step_size=deltaT)
...
TOTAL_EPOCHS = NUM_SEEDS * MAX_EPOCHS * len(k_options)
...
if __name__ == '__main__':
    ...
    for j, K in enumerate(k_options):
        ...
        loss_fun = create_loss_function(k = K)
        test_mesh, test_solution = generate_test(k = K)

        for init_seed in INIT_SEEDS:
            pbar.set_description(f"Processing {K} seed {init_seed}/{NUM_SEEDS-1}")
            ...
            seed_folder_name = os.path.join(result_dir, f"k_{K}", f"seed_{init_seed}")
