...
def create_loss_function(num_points = (51, 51), delta_t = 1e-3):
    mesh, boundaries = generate_mesh_object(num_points, domain=problem_domain, device=device, full_requires_grad=True, border_requires_grad=False, num_seq_steps=4, seq_step_size=delta_t)
...
def generate_test(num_points = (201, 201), delta_t = 1e-3):
    test_mesh, _ = generate_mesh_object(num_points, domain=problem_domain, device=device, full_requires_grad=False, border_requires_grad=False, num_seq_steps=4, seq_step_size=delta_t)
...
delta_t_options = [0.05, 0.01, 0.005, 0.001]
#model_test = [(test_mesh, analytic_solution), (test_mesh, analytic_solution), (single_test_mesh, single_analytic_solution)]
optimizer = LBFGS
MAX_EPOCHS = 100


TOTAL_EPOCHS = NUM_SEEDS * MAX_EPOCHS * len(delta_t_options)

if __name__ == '__main__':
    pbar = tqdm(total=TOTAL_EPOCHS, ncols=100)

    for j, deltaT in enumerate(delta_t_options):

        loss_fun = create_loss_function(delta_t = deltaT)
        test_mesh, test_solution = generate_test(delta_t = deltaT)

        for init_seed in INIT_SEEDS:
            pbar.set_description(f"Processing {deltaT} seed {init_seed}/{NUM_SEEDS-1}")
            ...
            seed_folder_name = os.path.join(result_dir, f"k4_deltaT_{deltaT}", f"seed_{init_seed}")
