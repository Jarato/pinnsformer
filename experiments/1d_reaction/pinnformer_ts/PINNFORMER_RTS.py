...
def loss_fn(model, mesh, b_left, b_right, initial, initial_values):
    mesh_domain_mask = mesh.full[:,:,-1] >= 0.0
    u = f(model, mesh)

    pde_residue = torch.flatten(df(model, mesh, wrt=1) - RHO*u*(1.0-u), start_dim=1)*mesh_domain_mask
    pde_loss = torch.sum(pde_residue.pow(2))/torch.sum(mesh_domain_mask)

    # boundary
    boundary_domain_mask = b_left.full[:,:,-1] >= 0.0
    boundary_residue = torch.flatten(f(model, b_left) - f(model, b_right), start_dim=1)*boundary_domain_mask
    boundary_loss = torch.sum(boundary_residue.pow(2))/torch.sum(boundary_domain_mask)
...
NUM_SEEDS = 4
INIT_SEEDS = np.array(range(NUM_SEEDS))+96
optimizer = LBFGS
MAX_EPOCHS = 100
